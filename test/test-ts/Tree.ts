...
export class AVLTree<T> extends BinarySearchTree<T> {
	...
	keepBalance(node: Node<T>): Node<T> {
		...
		if (balanceState === BalanceFactor.UNBALANCED_LEFT) {
			if (
				this.getBalanceFactor(node.left) === BalanceFactor.UNBALANCED_LEFT ||
				this.getBalanceFactor(node.left) === BalanceFactor.SLIGHTY_UNBALANCED_LEFT
			) {
				return this.rotationLL(node);
			} else if (
				this.getBalanceFactor(node.left) === BalanceFactor.UNBALANCED_RIGHT ||
				this.getBalanceFactor(node.left) === BalanceFactor.SLIGHTY_UNBALANCED_RIGHT
			) {
			...
			if (
				...
				this.getBalanceFactor(node.right) === BalanceFactor.SLIGHTY_UNBALANCED_RIGHT
			) {
				return this.rotationRR(node);
